        ...
        <div className="hidden md:flex items-center gap-2">
          {user ? (
            <>
              ...
              <Link href="/dashboard"
                className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${pathname === '/dashboard'
                    ? 'bg-[var(--color-bg-hover)] text-[var(--color-text)]'
                    : 'text-[var(--color-text-secondary)] hover:text-[var(--color-text)] hover:bg-[var(--color-bg-hover)]'
                  }`}
              >
              ...
              <div className="relative pl-3 ml-1 border-l border-[var(--color-border)]" ref={dropdownRef}>
                ...
                >
                  ...
                  <svg width="12" height="12" viewBox="0 0 12 12" fill="none" className={`text-[var(--color-text-muted)] transition-transform ${dropdownOpen ? 'rotate-180' : ''}`}>
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-[var(--color-border)] rounded-lg shadow-lg py-1 z-50">
                    ...
                    >
                      <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><circle cx="7" cy="4.5" r="2.5" stroke="currentColor" strokeWidth="1.2" /><path d="M2.5 12.5C2.5 10 4.5 8.5 7 8.5s4.5 1.5 4.5 4" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" /></svg>
                      {t('nav.myProfile')}
                    </Link>
                    ...
                    <Link href="/settings/profile"
                      ...
                      className="flex items-center gap-2 px-3 py-2 text-sm text-[var(--color-text-secondary)] hover:text-[var(--color-text)] hover:bg-[var(--color-bg-hover)] transition-colors"
                    >
                      <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><circle cx="7" cy="7" r="2" stroke="currentColor" strokeWidth="1.2" /><path d="M7 1v1.5M7 11.5V13M1 7h1.5M11.5 7H13M2.75 2.75l1.06 1.06M10.19 10.19l1.06 1.06M11.25 2.75l-1.06 1.06M3.81 10.19l-1.06 1.06" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" /></svg>
                      {t('nav.settings')}
                    </Link>
                    ...
                    <div className="border-t border-[var(--color-border-light)] mt-1 pt-1">
                      ...
                      >
                        <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M5 7h7m0 0L10 5m2 2l-2 2M8 10v1.5a1 1 0 01-1 1H3a1 1 0 01-1-1v-9a1 1 0 011-1h4a1 1 0 011 1V5" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" /></svg>
                        {t('nav.logout')}
                      </button>
                    </div>
                  </div>
            ...
            <>
              <Link href="/login" className="px-3 py-1.5 text-sm text-[var(--color-text-secondary)] hover:text-[var(--color-text)] transition-colors">{t('nav.login')}</Link>
              <Link href="/register" className="px-4 py-1.5 bg-[var(--color-brand)] hover:bg-[var(--color-brand-hover)] text-white text-sm font-medium rounded-lg transition-colors">{t('nav.signup')}</Link>
            </>
          )}
        </div>
          ...
          <div className="flex flex-col gap-0.5">
            {navLinks.map((link) => {
              ...
              return (
                <Link key={link.href} href={link.href} onClick={() => setMobileOpen(false)}
                  className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${isActive ? 'bg-[var(--color-bg-hover)] text-[var(--color-text)]' : 'text-[var(--color-text-secondary)] hover:text-[var(--color-text)]'
                    }`}
                >
                  {t(link.labelKey)}
                </Link>
            ...
            <div className="border-t border-[var(--color-border-light)] mt-2 pt-2">
              {user ? (
                ...
                <>
                  <Link href="/login" onClick={() => setMobileOpen(false)} className="block px-3 py-2 text-sm text-[var(--color-text-secondary)]">{t('nav.login')}</Link>
                  <Link href="/register" onClick={() => setMobileOpen(false)} className="block px-3 py-2 text-sm text-[var(--color-brand)] font-medium">{t('nav.signup')}</Link>
                </>
              )}
            </div>
          </div>

...
export default function CommentSection({ targetType, targetId }: CommentSectionProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const comment = await apiCreateComment(targetType, targetId, newComment.trim());
  ...
  return (
    <div className="mt-8 pt-6 border-t border-[var(--color-border-light)]">
      <h3 className="text-base font-semibold text-[var(--color-text)] mb-4">
        {t('comments.title')} {!isLoading && <span className="text-[var(--color-text-muted)] font-normal text-sm">({comments.length})</span>}
      </h3>

      {user ? (
        <form onSubmit={handleSubmit} className="mb-6">
          <textarea
            id="new-comment" name="comment" value={newComment} onChange={(e) => setNewComment(e.target.value)}
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[1, 2].map((i) => (
            <div key={i} className="animate-pulse rounded-lg border border-[var(--color-border)] bg-white p-4">
        ...
        <p className="text-sm text-[var(--color-text-muted)]">{t('comments.noComments')}</p>
      ) : (
        <div className="space-y-3">
          {comments.map((comment) => (
            <div key={comment._id} className="rounded-lg border border-[var(--color-border)] bg-white p-4">
              <div className="flex items-center justify-between mb-1.5">
                ...
                {user && comment.author && (user as any)._id === comment.author._id && (
                  <button onClick={() => handleDelete(comment._id)} className="text-xs text-[var(--color-text-muted)] hover:text-red-500 transition-colors">{t('comments.delete')}</button>
                )}
              </div>

...
import {
  ...
  apiUpdateSubStep,
  apiRemoveSubStep,
  apiUpdateStepVocabularies,
  apiUpdateSubStepVocabularies,
  apiUpdateRoadmap,
  apiUpdateStep,
...
export default function RoadmapDetailPage() {
  ...
  const [editSubStepTarget, setEditSubStepTarget] = useState<{
    step: number;
    subStep: number;
  } | null>(null);
  ...
  const fetchRoadmap = useCallback(async () => {
    try {
      const data = await apiGetRoadmap(id);
      setRoadmap(data);
    } catch {
    } finally {
      setIsLoading(false);
  ...
  const handleAddStep = async () => {
    ...
    try {
      const updated = await apiAddStep(id, { title: newStepTitle.trim() });
      setRoadmap(updated);
      setNewStepTitle("");
    } catch {}
  };
  ...
  const handleAddSubStep = async (stepIndex: number) => {
    ...
    try {
      const updated = await apiAddSubStep(id, stepIndex, {
  ...
  const handleEditSubStep = async (stepIndex: number, subStepIndex: number) => {
    ...
    try {
      const updated = await apiUpdateSubStep(id, stepIndex, subStepIndex, {
        title: editSubStepTitle.trim(),
      });
      setRoadmap(updated);
  ...
  const handleDeleteSubStep = async (
    stepIndex: number,
    subStepIndex: number,
  ) => {
  ...
  const handleAddVocabulary = async (
    ...
    const newVocabList = [
      ...existingVocabs,
      { front: newVocabFront.trim(), back: newVocabBack.trim() },
    ];

    try {
      let updated;
      if (subStepIndex !== undefined) {
        updated = await apiUpdateSubStepVocabularies(
          id,
          stepIndex,
          subStepIndex,
          newVocabList,
  ...
  const handleUpdateRoadmapDeadline = async (date: Date | undefined) => {
    if (!date) return;
    try {
      const updated = await apiUpdateRoadmap(id, {
  ...
  const handleUpdateStepDeadline = async (
    stepIndex: number,
    date: Date | undefined,
  ) => {
    if (!date) return;
    try {
      const updated = await apiUpdateStep(id, stepIndex, {
        deadline: date.toISOString(),
      });
      setRoadmap(updated);
    } catch {}
  ...
  const handleExportAnki = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `anki-export-${roadmap.title.replace(/[^a-z0-9]/gi, "_").toLowerCase()}.csv`,
      );
  ...
  const isOwner =
    ...
    roadmap.owner &&
    (user as any)._id ===
      (typeof roadmap.owner === "string" ? roadmap.owner : roadmap.owner._id);
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mt-3 text-xs text-[var(--color-text-muted)]">
              ...
              <span className="flex items-center gap-1">
                <Clock size={14} /> Créée {daysString}
              </span>
              <span>
                · {completedCount}/{roadmap.steps.length}{" "}
                {t("roadmaps.stepsCompleted")}
              </span>
            </div>
            {isOwner && (
              <div className="mt-3 flex items-center gap-2 text-sm">
                ...
                >
                  <PopoverTrigger className="flex items-center gap-2 px-3 py-1.5 text-xs text-left border rounded bg-white text-[var(--color-text)] hover:border-[var(--color-brand)] transition-colors">
                    ...
                    <span>
                      {roadmap.deadline
                        ? format(new Date(roadmap.deadline), "PPP", {
                            locale: fr,
                          })
      ...
      <div className="space-y-3 mb-6">
        {roadmap.steps.map((step, stepIndex) => {
          ...
          const isTogglingStep =
            toggling?.step === stepIndex && toggling?.subStep === undefined;
            ...
            >
              ...
              >
                <div onClick={(e) => e.stopPropagation()}>
                  {isOwner ? (
                    <button
                      onClick={() => handleToggle(stepIndex)}
                      disabled={isTogglingStep}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-3 mt-2 text-xs text-[var(--color-text-muted)]">
                    {isOwner ? (
                      ...
                      >
                        <Popover
                          open={stepCalendarOpen === stepIndex}
                          onOpenChange={(open) =>
                            setStepCalendarOpen(open ? stepIndex : null)
                          }
                        >
                    ...
                    {step.vocabularies && step.vocabularies.length > 0 && (
                      <span className="flex items-center gap-1">
                        <BookOpen size={12} /> {step.vocabularies.length} words
                      </span>
                    )}
                    {step.completedAt && (
                  ...
                  <div className="mb-5">
                    ...
                    <div className="space-y-2 mb-3">
                      {step.subSteps?.map((subStep, subIdx) => {
                        const isTogglingSub =
                          toggling?.step === stepIndex &&
                          toggling?.subStep === subIdx;
                        return (
                          <div
                            key={subIdx}
                          ...
                          >
                            ...
                            <div className="flex-1">
                              {isOwner &&
                              editSubStepTarget?.step === stepIndex &&
                              editSubStepTarget?.subStep === subIdx ? (
                                <div className="flex items-center gap-2 animate-in fade-in slide-in-from-top-1 w-full max-w-[80%] pr-4">
                                  <Input
                                    autoFocus
                                    value={editSubStepTitle}
                                    onChange={(e) =>
                                      setEditSubStepTitle(e.target.value)
                                    }
                    ...
                    {isOwner && newSubStepTarget === stepIndex ? (
                      <div className="flex gap-2 pl-4 border-l-2 border-[var(--color-brand)] animate-in fade-in slide-in-from-top-2">
                        <Input
                          ...
                          onKeyDown={(e) =>
                            e.key === "Enter" && handleAddSubStep(stepIndex)
                          }
                        />
                        <button
                      ...
                      <div className="bg-white p-3 rounded border border-[var(--color-brand)]/30 animate-in fade-in slide-in-from-top-2">
                        <div className="grid grid-cols-2 gap-2 mb-2">
                          <Input
                            autoFocus
                            value={newVocabFront}
                            onChange={(e) => setNewVocabFront(e.target.value)}
                            placeholder="Word (Front)"
                            className="h-8 text-sm"
                          />
                          <Input
                            value={newVocabBack}
                            onChange={(e) => setNewVocabBack(e.target.value)}
                            placeholder="Translation (Back)"
                            className="h-8 text-sm"
                            onKeyDown={(e) =>
                              e.key === "Enter" &&
                              handleAddVocabulary(stepIndex)
                            }
                          />
                        </div>
                        <div className="flex gap-2">
                          <button
                            onClick={() => handleAddVocabulary(stepIndex)}
                            disabled={

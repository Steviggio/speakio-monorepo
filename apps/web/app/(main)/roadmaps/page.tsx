...
export default function RoadmapsPage() {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      try {
        const data = await apiGetMyRoadmaps();
        setRoadmaps(data);
      } catch (error) {
  ...
  const handleCreate = async () => {
    ...
    try {
      const roadmap = await apiCreateRoadmap({
        title: newTitle.trim(),
  ...
  const getProgress = (steps: { completed: boolean }[]) =>
    steps.length === 0
      ? 0
      : Math.round(
        ...
        <div className="rounded-lg border border-[var(--color-border)] bg-white p-5 shadow-sm mb-6">
          ...
          <div className="space-y-3">
            <Input
              ...
              placeholder={t("roadmaps.roadmapTitle")}
            />
            <Input
              type="text"
              id="roadmap-desc"
              ...
              placeholder={t("roadmaps.roadmapDesc")}
            />
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <Button
                      ...
                      className="text-red-500 hover:text-red-600 transition-colors px-2 h-auto text-xs py-1"
                    >
                      {t("roadmaps.deleteRoadmap")}
                    </Button>
                  </div>
                  <div className="mt-3">

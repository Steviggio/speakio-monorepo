...
export default function ProfilePage() {
  ...
  if (error || !profile) return (
    <div className="max-w-3xl mx-auto py-8 px-4 text-center">
      <p className="text-[var(--color-text-muted)]">{t('profile.notFound')}</p>
      <Link href="/dashboard" className="text-[var(--color-brand)] mt-3 inline-block text-sm">{t('profile.backToDashboard')}</Link>
    </div>
  );
        ...
        <div className="flex flex-col md:flex-row items-center gap-6">
          ...
          <div className="flex-1 text-center md:text-left">
            <div className="flex flex-col md:flex-row md:items-center gap-2 mb-2">
              <h1 className="text-2xl font-bold text-[var(--color-text)]">{profile.username}</h1>
              {isOwnProfile && (
                <Link href="/settings/profile" className="text-xs text-[var(--color-brand)] hover:text-[var(--color-brand-hover)] transition-colors font-medium">
            ...
            <div className="mt-3 flex flex-wrap gap-1.5 justify-center md:justify-start">
              {profile.learningLanguages?.map((lang: string) => (
                <span key={lang} className="px-2.5 py-1 bg-[var(--color-bg)] text-[var(--color-text-secondary)] rounded-md text-xs font-medium border border-[var(--color-border-light)] uppercase">
                  {lang}
                </span>
              ))}
            ...
            {profile.createdAt && (
              <p className="mt-3 text-xs text-[var(--color-text-muted)]">
                {t('profile.memberSince')} {new Date(profile.createdAt).toLocaleDateString(locale || 'en-US', { month: 'long', year: 'numeric' })}
              </p>
            )}
          </div>
        </div>

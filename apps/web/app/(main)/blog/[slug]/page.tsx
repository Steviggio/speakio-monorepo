...
function renderMarkdown(md: string): string {
  return (md || '')
    ...
    .replace(
      /^## (.+)$/gm,
      '<h2 class="text-xl font-bold text-[var(--color-text)] mt-8 mb-3">$1</h2>',
    )
    .replace(
      /^# (.+)$/gm,
      '<h1 class="text-2xl font-bold text-[var(--color-text)] mt-8 mb-3">$1</h1>',
    )
    .replace(
      /\*\*(.+?)\*\*/g,
    ...
    .replace(
      /`(.+?)`/g,
      '<code class="bg-[var(--color-bg)] px-1.5 py-0.5 rounded text-sm text-[var(--color-brand)]">$1</code>',
    )
    .replace(
      /\[(.+?)\]\((.+?)\)/g,
      '<a href="$2" target="_blank" rel="noopener noreferrer" class="text-[var(--color-brand)] hover:text-[var(--color-brand-hover)] underline">$1</a>',
    )
    .replace(
      /^- (.+)$/gm,
  ...
  return (
    <article className="max-w-3xl mx-auto py-8 px-4">
      ...
      {showDeleteConfirm && (
        ...
        >
          <div
            className="bg-white rounded-lg border border-[var(--color-border)] shadow-xl p-6 max-w-sm mx-4"
      ...
      <header className="mb-8">
        <h1 className="text-2xl sm:text-3xl font-extrabold text-[var(--color-text)] leading-tight mb-3">
          {post.title}
        </h1>

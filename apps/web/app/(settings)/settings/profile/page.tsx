...
export default function ProfileSettingsPage() {
  ...
  const onSubmit = async (data: ProfileFormValues) => {
    ...
    try {
      ...
      if (data.learningLanguagesString) {
        payload.learningLanguages = data.learningLanguagesString
          .split(",")
          .map((l) => l.trim())
          .filter((l) => l.length > 0) as LanguageCode[];
      } else {
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (user)
        login(Cookies.get("access_token") || "", { ...user, avatarUrl });
    } catch (err) {
      const e = err as any;
      setErrorMsg(e.response?.data?.message || t("settings.updateFailed"));
    } finally {
      ...
      <Card className="p-6">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input
              label={t("settings.learningLanguages")}
              placeholder="en, fr, es"
              {...register("learningLanguagesString")}
              error={errors.learningLanguagesString?.message}
            />
            <Input
              label={t("settings.nativeLocale")}
              placeholder="en"
              {...register("locale")}

...
export default function AccountSettingsPage() {
  ...
  const handleChangeEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user)
        login(response.data.access_token || Cookies.get("access_token") || "", {
          ...user,
          email: newEmail,
        });
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPasswordError(
        err.response?.data?.message || t("settings.passwordFailed"),
      );
    } finally {
      ...
      <Card className="p-6">
        ...
        <form onSubmit={handleChangeEmail} className="space-y-3">
          ...
          <div className="flex justify-end">
            <Button type="submit" size="sm" isLoading={emailLoading}>
        ...
        {!showDeleteConfirm ? (
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 space-y-3">
            <p className="text-sm text-red-600">
              {t("settings.deleteConfirmPrompt").replace("{confirm}", "")}{" "}

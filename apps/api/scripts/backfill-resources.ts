...
function hasMeaningfulSourcePlatform(resource: ResourceDocument | any): boolean {
    return Boolean(
    ...
    try {
        ...
        for (const resource of docs) {
            ...
            try {
                ...
                if (!resource.canonicalUrl || resource.canonicalUrl !== normalized.canonicalUrl) {
                    $set.canonicalUrl = normalized.canonicalUrl;
                }

                if (
                    !hasMeaningfulSourcePlatform(resource) ||
                    resource.sourcePlatform?.domain !== normalized.sourcePlatform.domain ||
                    resource.sourcePlatform?.rootDomain !== normalized.sourcePlatform.rootDomain ||
                    resource.sourcePlatform?.baseUrl !== normalized.sourcePlatform.baseUrl ||
